...
async function run() {
  ...
  // Mark Alice as paid
  markPaid(recipients[0], 50, 50);
  const dueAfter = getDueRecipients(recipients);
  assert(dueAfter.length === 2, "2 recipients due after paying Alice (monthly)");
  assert(recipients[0].history.length === 1, "Alice has 1 payment in history");
  assert(recipients[0].history[0].amountZec === 50, "Alice history records 50 ZEC");

  // Mark Carol (one-time) as paid
  markPaid(recipients[2], 10, 50);
  assert(recipients[2].paid === true, "Carol marked as paid (one-time)");
  const dueAfter2 = getDueRecipients(recipients);
